...
const self = module.exports = {
  ...
  handler: (event, context, callback) => {
    ...
    const handleCampaignData = async () => {
      try {
        ...
        const lastSuccessfulDate = await getAsync(LAST_SUCCESS_KEY)
  ...
  trackEvents: async (formattedDate, type) => {
    ...
    const fileName = await self.determineFileName(type, formattedDate)
    if (fileName) {
      console.log(`Begin getDataFromS3 for ${fileName}`)
      const csvData = await self.getDataFromS3(fileName)
      console.log(`Begin parseDataFromCsv for ${fileName}`)
      const parsedData = await self.parseDataFromCsv(csvData)
